...
🔗 **在线访问**：<https://qhlstudyio-stock-screener.streamlit.app>
...
- **主题切换**：切换 Light/Dark 主题后，需要在侧边栏进行一次交互（如移动滑块）才能刷新图表配色。这是 Streamlit 框架本身的限制。

- **数据持久化**：数据存储在 SQLite 中。Streamlit Cloud 重启后数据库会重置，需点击「Refresh Data」重新抓取数据。
